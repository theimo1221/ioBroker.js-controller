...
function getRepositoryFile(urlOrPath, additionalInfo, callback) {
    ...
    // If object was read
    if (urlOrPath && typeof urlOrPath === 'object') {
        typeof callback === 'function' && callback(null, urlOrPath);
    } else
    if (!urlOrPath) {
        try {
    ...
    objects.getObject('system.config', (_err, systemConfig) => {
        objects.getObject('system.repositories', (err, repos) => {
            ...
            if (!err && repos && repos.native && repos.native.repositories) {
                ...
                active.forEach(repo => {
                    if (repos.native.repositories[repo] && repos.native.repositories[repo].json) {
                        Object.assign(result, repos.native.repositories[repo].json);
                    }
                });

                if (result) {
                    data['adapters count'] = Object.keys(result).length;
                }
            }
            if (!--task) {
                callback(err, data);
