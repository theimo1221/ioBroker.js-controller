        ...
        if (!repoUrl || typeof repoUrl !== 'object') {
            ...
            return objects.getObject('system.config', (err, sysConfig) => {
                objects.getObject('system.repositories', (err, obj) => {
                    ...
                    if (!force) {
                        if (err || !obj) {
                            ...
                            if (!obj.native || !obj.native.repositories) {
                                ...
                                // TODO: multiple REPO
                                let activeRepo = sysConfig.common.activeRepo;
                                if (typeof activeRepo !== 'object') {
                                    activeRepo = [activeRepo];
                                }

                                activeRepo.forEach(repo => {
                                    // If known repository
                                    if (obj.native.repositories[repo]) {
                                        if (typeof obj.native.repositories[repo] === 'object') {
                                            additionalInfo.hash = obj.native.repositories[repo].hash;
                                            additionalInfo.sources = obj.native.repositories[repo].json;
                                        }
                                    }
                                });
                            }
                    ...
                    tools.getRepositoryFile(repoUrl, additionalInfo, (err, sources, sourcesHash) =>
                        updateRepo(sources, sourcesHash, force, callback));
                });
            });
        }
            ...
            if (err || !obj) {
                ...
                if (obj.native.repositories) {
                    ...
                    objects.getObject('system.config', (err, obj) => {
                        let active = obj.common.activeRepo;
    ...
    this.rename = async function (oldName, newName, matchingLink) {
        ...
        if (repoObj && repoObj.native && repoObj.native.repositories) {
            if (repoObj.native.repositories[oldName] && repoObj.native.repositories[oldName].link === matchingLink &&
                ...
                let active = sysConfigObj.common.activeRepo;
                // convert old format to new one
                if (!Array.isArray(active)) {
                    active = [active];
                }
                ...
                if (sysConfigObj && sysConfigObj.common && active.includes(oldName)) {
                    const pos = active.indexOf(oldName);
                    active[pos] = newName;
                    sysConfigObj.common.activeRepo = active;
                    try {
